...
MODULE_DESCRIPTION("tp-multi-instances");
MODULE_AUTHOR("TG");
MODULE_LICENSE("GPL");
 ...
 * Arguments
 */
static int major = 0; /* Automatic major number */
module_param(major, int, 0660);
MODULE_PARM_DESC(major, "Static major number (none = 0 = dynamic)");
 ...
 * Global variables
 */
static char*  buffer = NULL; /* The buffer                              */
...
static int k_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_DEBUG "char: open() by minor %d\n", minor);
...
static int k_release(struct inode *inode, struct file *file)
{
	...
	printk(KERN_DEBUG "char: release() by minor %d\n", minor);
