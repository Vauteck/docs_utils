...
static int __init my_init (void)
{
    ...
    printk (KERN_INFO " rc from call chain = %d\n", rc);
    printk (KERN_INFO "Notifier module successfully loaded\n");
...
static void __exit my_exit (void)
{
    ...
    if (blocking_notifier_chain_unregister (&my_nh, &my_nh_block))
        printk (KERN_INFO " Failed to unregister from notifier\n");
    printk (KERN_INFO "Notifier module successfully unloaded\n");
}
...
MODULE_DESCRIPTION ("CLDD:1.0 s_31/lab2_notifier.c");
