...
{
    printk (KERN_INFO " RELEASING my_mouse device: %s:", MYDEV_NAME);
    printk (KERN_INFO "  MAJOR number = %d, MINOR number = %d\n",
            imajor (inode), iminor (inode));
    return 0;
}
...
mymouse_read (struct file *file, char __user * buf, size_t lbuf, loff_t * ppos)
{
    ...
    printk (KERN_INFO "xpos=%d, ypos=%d\n", xpos, ypos);
...
static int
mymouse_ioctl (struct inode *inode, struct file *fp,
               unsigned int cmd, unsigned long arg)
{
    switch (cmd) {
    case MY_IOC_Z:
        printk (KERN_INFO " zeroing out x and y, which were x= %d, y= %d\n",
                xpos, ypos);
        xpos = 0;
        ypos = 0;
        break;
    default:
        printk (KERN_INFO " got unknown ioctl, CMD=%d\n", cmd);
        return -EINVAL;
    }
    return 0;
}
...
static irqreturn_t my_interrupt (int irq, void *dev_id)
{
    ...
    if (nbytes_in == 3) {
        ...
        xpos += dx;
        ypos += dy;
        printk (KERN_INFO "mymouse_int %d", interrupt_number);
        printk (KERN_INFO "  Data = %2x:%2x:%2x (buttons[%c%c]), dx=%d, dy=%d",
                byte_in[0], byte_in[1], byte_in[2], left, right, dx, dy);
...
static int __init my_init (void)
{
    if (request_irq
        ...
        return -1;
    printk (KERN_INFO " my_mouse INTERRUPT %d successfully registered\n",
            MOUSE_IRQ);
    ...
    if (!(my_cdev = cdev_alloc ())) {
        printk (KERN_ERR "failed to alloc_cdev\n");
    ...
    printk (KERN_INFO " turning on the mouse\n");
    mymouse_turnon ();
    return 0;
}
...
MODULE_DESCRIPTION ("CLDD:1.0 s_21/lab4_serial.c");
