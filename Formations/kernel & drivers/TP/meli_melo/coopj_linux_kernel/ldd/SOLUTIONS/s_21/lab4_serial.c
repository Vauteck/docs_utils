...
mymouse_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case MY_IOC_Z:
		printk(KERN_INFO
		       " zeroing out x and y, which were x= %d, y= %d\n", xpos,
		       ypos);
		xpos = 0;
		ypos = 0;
		break;
...
static irqreturn_t my_interrupt(int irq, void *dev_id)
{
	...
	if (nbytes_in == 3) {
		...
		printk(KERN_INFO
		       "  Data = %2x:%2x:%2x (buttons[%c%c]), dx=%d, dy=%d",
...
static int __init my_init(void)
{
	...
	if (!(my_cdev = cdev_alloc())) {
		printk(KERN_ERR "failed to cdev_alloc\n");
...
MODULE_DESCRIPTION(""":1.0 s_21/lab4_serial.c");
