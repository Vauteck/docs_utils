...
#include <asm/io.h>
	...
	static char * nom_module = "exemple_16_B";
	...
	static unsigned int poll_exemple (struct file * filp, poll_table * attente);
...
static unsigned int poll_exemple  (struct file * filp, poll_table * attente)
{
	...
	poll_wait(filp, & waitqueue_buffer_exemple, attente);
	spin_lock_bh(& lock_buffer_exemple);
	if (fin_buffer_exemple > 0)
